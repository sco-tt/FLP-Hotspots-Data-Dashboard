...
"use strict";
	...
	/**
	  * Using jQuery Event Handlers to look at the 'Hotspot' and 'FY' checkbox sets. 
	  * Add/remove values to hotspotQueryParams array 
	**/ 
	...
	$("#search").on("click", function() {
		...
		else if (hotspotQueryParams[1].length > 0) {
			selectedFY += "(";
			var selectedFY = "(";
	...
	/**
	 * Build our query url depending on the 
	 * parameters that are checked in the form
	**/

	function queryBuilder(selectedHotspots, selectedFY) {
		clearTable();
		
		// Alway include a year query; too slow otherwise
		...
		// none
		if (!(selectedHotspots) && !(selectedFY)) { 
	...
	function search (data_uppercase) {
		...
		for (var i = 0, len = data_uppercase.length; i < len; i++) {
		 ...
		 /**
		  * Check if at least of one of our tag 
		  * columns has a value and logged tag visits
		  */
		  ...
		  /**
		   * Test each row against our index 
		   * and write to the array, acting as a count
		   */
		  var counter = 0;
		  ...
		  for (var j=0, len2 = tagList.length; j < len2; j++) { 
		  	...
		  	//loop through each categorized tag
		  	for (var k = 0, len3 = tagList[j][2].length; k < len3; k++)	{ 
